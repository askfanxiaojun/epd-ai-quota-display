...
后续真实数据出现 `99%` 时，又暴露出一个字体边界问题：Pillow 接收的文字 Y
坐标是字体排版框的起点，而不是肉眼看到的字形顶部。PingFang 33px 数字的
可见区域会比传入坐标低约 12px，因此两位数末端会压到进度条上。最终处理为：

- 百分比数字整体上移 9px；
- 进度条整体下移 2px；
- 百分号保持与数字基线对齐；
- 使用左右两个 `99%` 窗口进行压力测试；
- 在实体屏上使用真实的 `88% / 74%` 数据再次验证。

修改后数字与进度条之间保留了清晰的实际像素间距，同时没有挤压重置时间。
