...
> [!WARNING]
> **实时温度与湿度功能目前存在稳定性问题。** 暂时不要刷入相关固件，也不建议
> 启用 `calendar-sensor` 自动更新。相关代码、命令和预览仅保留用于后续调试；
> 当前请优先使用稳定的 `calendar-agenda` 月历日程页面。
...
| **设备独立日历温度（实验）** | 当前存在稳定性问题，暂时不要刷入相关固件；实现仅保留用于后续调试。 |
| **外部温湿度模式（实验）** | 当前暂不建议启用自动更新；传感器入口和页面预览仅供开发测试。 |
...
> [!CAUTION]
> 此模式目前存在问题，**暂时不要刷入配套固件，也不要执行下面的启用命令**。
> 以下内容仅作为技术记录保留。
...
> [!WARNING]
> 实时温湿度页面目前仍是实验功能，暂不建议接入自动刷新任务或写入日常使用的
> 墨水屏。请先继续使用 `calendar-agenda`。
...
> 当前仅供本地开发和 `--dry-run` 预览，请勿作为正式写屏配置。
...
设备独立日历温度模式要求版本 `0x21` 或更高，但当前版本存在问题。下面的命令
仅供后续开发调试，**现在不要执行，也不要刷入相关固件**：
...
墨水屏刷新需要几秒钟。自动更新默认不会先清除旧画面，而是发送完整黑色层和
红色层后再触发刷新；只有人工指定 `--clear-first` 时才会先清屏并重新初始化。
...
| `--device-calendar-temperature` | 实验功能；当前存在问题，暂勿执行或刷入相关固件 |
...
| `--mode calendar-sensor` | 实验功能；仅建议 `--dry-run` 预览，暂勿用于自动写屏 |
